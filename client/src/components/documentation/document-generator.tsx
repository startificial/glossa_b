...
${requirements.map(req => 
  `### ${req.category.toUpperCase()}: ${req.description}
...
${functionalReqs.map((req, index) => 
  `### ${index + 1}. ${req.description.split('.')[0] || req.description}
${req.description}
...
${requirements.filter(req => req.category.toLowerCase() === 'functional').map((req, index) => 
  `### Module ${index + 1}: ${req.description.split('.')[0] || req.description}
**Duration**: 30 minutes
**Objective**: Learn how to ${req.description.toLowerCase().startsWith('ability to') ? req.description.substring(11) : req.description.split('.')[0].toLowerCase()}
...
## Key Features
${functionalReqs.map((req, index) => 
  `### Feature ${index + 1}: ${req.description.split('.')[0] || req.description}
...
${functionalReqs.slice(0, 5).map(req => 
  `| ${req.description.split('.')[0] || req.description} | ✓ | ✓ | ✓ |`
).join('\n')}
