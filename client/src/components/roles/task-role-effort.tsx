...
interface TaskRoleEffortProps {
  projectId: number;
  taskId: number;
}

export function TaskRoleEffort({ projectId, taskId }: TaskRoleEffortProps) {
  ...
  const addRoleEffortMutation = useMutation({
    mutationFn: async (data: { roleId: number; estimatedEffort: string; effortUnit: string }) => {
      return await apiRequest(`/api/tasks/${taskId}/role-efforts`, {
        method: "POST",
        data,
      });
    },
    onSuccess: () => {
      toast({
        ...
        description: "Role effort has been added to the task",
      });
  ...
  const deleteRoleEffortMutation = useMutation({
    mutationFn: async (effortId: number) => {
      return await apiRequest(`/api/tasks/${taskId}/role-efforts/${effortId}`, {
        method: "DELETE",
      });
    },
    onSuccess: () => {
      toast({
        ...
        description: "Role effort has been removed from the task",
      });
      queryClient.invalidateQueries({
        queryKey: [`/api/tasks/${taskId}/role-efforts`],
      });
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Estimate the effort required from specific roles to implement this task
        </CardDescription>
      </CardHeader>
            ...
            {hasAssignedRoles ? (
              ...
              <div className="flex flex-col items-center justify-center py-6 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1 mb-4 max-w-md">
                  Assign roles to estimate effort required for implementing this task
                </p>
